...
export const ImagePost = <T,>({
  ...
  const { data: video } = useQuery<PostResponse<YtdlItem>>({
    ...
    queryKey: [
      `/posts/${data?.videoUrl && encodeURIComponent(data?.videoUrl)}`,
    ],
  });
    ...
    if (videoRef.current && videoExtractor && video) {
      ...
      if (player) {
        player.configure({
          ...
          preferreddAudioCodecs: ["opus", "mp4a"],
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-auto bg-black text-white box-border lg:p-8">
      <div className="w-full lg:h-[550px] lg:w-4/5 lg:border border-neutral-600 flex-auto">
        <div className="h-[calc(100dvh-60px)] lg:h-full flex flex-auto flex-col md:flex-row">
          ...
          <div className="md:border-l border-neutral-600 lg:w-[300px] shrink-0 lg:p-0 flex flex-col gap-y-2 py-4 p-2">
            <div className="md:border-b border-neutral-600 lg:p-4 flex gap-x-2 items-center px-2">
